...
@router.post("/users/{user_id}/albums/", response_model=album_schema.AlbumCreate)
def create_album_for_user(
    user_id: int, album: album_schema.AlbumCreate, db: Session = Depends(get_session_local)):
    return album_logic.create_album(db=db, album=album, user_id=user_id)

@router.get("/albums/{album_id}", response_model=album_schema.AlbumBase)
...
@router.delete("/albums/{album_id}", response_model=album_schema.AlbumBase)
...
@router.get("/users/{user_id}/albums/", response_model=list[album_schema.AlbumBase])
